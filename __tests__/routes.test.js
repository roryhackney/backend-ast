...
describe('Login Test 2', () => {
    it('should fucking work', async () => {
        const response = await request(app).post('/login').send({username: process.env.ADMIN_USERNAME, password: process.env.ADMIN_PASSWORD});
        expect(response.status).toBe(200);
...
//         expect(res.body.superSecret).toBe('encryptedstring');
//     });
// });